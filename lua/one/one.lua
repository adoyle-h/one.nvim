...
function One.showConfig(opts)
	opts = vim.tbl_deep_extend('keep', opts, {
		fargs = {},
	})
	local util = One.util

	local title = '[one.nvim config]'
	local w = util.newWindow({ title = title, ft = 'lua' })
	local write = w.write
	write({ '-- The content generated by :OneShowConfig ' .. opts.args, '-- config --' })

	local config = vim.tbl_get(One.CM.config, table.unpack(opts.fargs))
