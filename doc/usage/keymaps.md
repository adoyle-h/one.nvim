...
- `<space>A` = Grep texts with ast_grep
...
- `=` = Format a buffer using the attached language server clients. (Format will be trigger when write buffer to disk)
- `=` in selection = Range format selected text using the attached language server clients.
